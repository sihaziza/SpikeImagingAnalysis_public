...
mainFolder='B:\Calibration\Raw\Noise';

for i=3:length(temp)
folder=dir(fullfile(mainFolder,temp(i).name,'20210120','meas*')); % G always comes before R
...
for iFolder=1:length(folderName)
    try
        dcimgPath=fullfile(mainFolder,temp(i).name,'20210120',folderName{iFolder});
...
mainFolder='F:\GECI\Preprocessed\Spontaneous';
mouse={'mGC1'};
% fail=[];
% date='20200416';
