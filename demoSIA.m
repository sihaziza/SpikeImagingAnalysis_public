...
mainFolder='F:\GEVI_Spike\Preprocessed\Visual';

answer=input('danger! batch file delete > are you sure [NO-0/YES-1]');
if answer
    [fileToDelete]=deleteAllTempH5(mainFolder);
else
    dips('oh my goodness, it was close!')
end

%% DELETE ALL TEMPORARY FILES

path='F:\GEVI_Spike\Preprocessed\Spontaneous\m915';
...
if answer
   for i=1:numel(filelist)
       path2delete=fullfile(filelist(i).folder,filelist(i).name);
       rmdir(path2delete,'s');
   end
else
    dips('oh my goodness, it was close!')
