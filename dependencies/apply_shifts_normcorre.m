function M_final = apply_shifts_normcorre(Y,shifts,options,td1,td2,td3,col_shift)
% debugged normcorre file for batch processing on h5 files % - 2020-06-30 11:25:59 -   RC
...
%             end
        case 'hdf5'
%             Ytm =
%             single(h5read(Y,data_name,[ones(1,length(sizY)-1),t],[sizY(1:end-1),min(t+bin_width-1,T)-t+1]));
               Ytm=h5readchunk(Y,[t,t+bin_width-1]); % RC fix
        case 'mem'
